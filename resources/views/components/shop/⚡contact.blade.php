...
new #[Layout('components.layouts.app')] class extends Component
{
    ...
    public function submit()
    {
        ...
        \Illuminate\Support\Facades\Mail::to(config('mail.from.address', 'hello@rituals.com'))->send(
            new \App\Mail\ContactMessage($this->name, $this->email, $this->message)
        );
    ...
    <div class="bg-white/5 p-10 rounded-[2rem] border border-white/10 backdrop-blur-xl">
        ...
        <form wire:submit="submit" class="space-y-6">
            <div>
                ...
                @error('name') <span class="text-sm text-red-400 mt-1 block font-accent">{{ $message }}</span> @enderror
            </div>
            ...
            <div>
                ...
                @error('email') <span class="text-sm text-red-400 mt-1 block font-accent">{{ $message }}</span> @enderror
            </div>
            ...
            <div>
                ...
                @error('message') <span class="text-sm text-red-400 mt-1 block font-accent">{{ $message }}</span> @enderror
            </div>
